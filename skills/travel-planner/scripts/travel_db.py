...
def ensure_db_files() -> None:
    ...
    if not TRIPS_FILE.exists():
        default_trips = {"current_trips": [], "past_trips": [], "trip_ideas": []}
...
def add_to_bucket_list(destination: str, notes: str = "") -> None:
    ...
    prefs["bucket_list"].append({"destination": destination, "notes": notes, "added_at": datetime.now().isoformat()})
    save_json(PREFERENCES_FILE, prefs)
...
def get_trips(status: str = "all") -> dict[str, list[dict]]:
    ...
    if status == "all":
        return trips
    if status in ["current", "past", "idea", "ideas"]:
        key = f"{status}_trips" if status not in ("idea", "ideas") else "trip_ideas"
...
def add_trip(trip: dict[str, Any], status: str = "current") -> str:
    ...
    Returns:
        Trip ID
    """
    valid_statuses = ("current", "past", "idea")
    if status not in valid_statuses:
        raise ValueError(f"Invalid trip status '{status}'. Must be one of: {', '.join(valid_statuses)}")
...
def export_all() -> dict[str, Any]:
    ...
    return {"preferences": get_preferences(), "trips": get_trips("all"), "stats": get_travel_stats(), "exported_at": datetime.now().isoformat()}
...
if __name__ == "__main__":
    ...
    if command == "is_initialized":
        ...
        status = sys.argv[2] if len(sys.argv) > 2 else "all"  # noqa: PLR2004
